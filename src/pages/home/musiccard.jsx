...
function MusicVideoCard(params) {
    ...
    const updatePlayingSong = (list) => {
        console.log(list.songUrl);
        updateSong(list.songUrl)
        updateName(list.name)
        updateDir(list.dir)
        updateViews(list.views)
        updateImage(list.image)
    }
    return (
        <>
            {
                Songlist.map((list, index) => {
                    return (
                        <div key={index} className="min-w-80">
                            <div className="group relative w-full after:absolute hover:cursor-pointer after:content-[''] after:top-0 after:left-0 after:h-full after:w-full hover:after:bg-black/40 " onClick={()=>updatePlayingSong(list)}>
                                <img src={list.image} alt="music" className="w-full  object-contain" />
